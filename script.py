...
weighted_sal.rename(columns = {'wt': 'Avg Num Players Per Team'}, inplace = True)
...
col1.dataframe(average_starter_sal[['Cap Hit', 'Base Salary', 'Unit']].round(0), height = 750)

col2.subheader('Average Weighted Compensation by Position', divider = 'gray')

col2.dataframe(weighted_sal[['Weighted Cap Hit', 'Avg Num Players Per Team', 'Unit']].round(1).sort_values('Weighted Cap Hit', ascending = False), use_container_width = True, height = 750)
